...
void mueve_lista_balas ( NodoPtr cabecera ) {
    ...
    while (actual != NULL) {
        actual->elem->x += actual->elem->velocidadBalax;
        actual->elem->y += actual->elem->velocidadBalay;

        // Si la bala sale de la pantalla, eliminarla de la lista
        if (actual->elem->x > 800 || actual->elem->x < 0 || actual->elem->y < 0 || actual->elem->y > 480) {
            NodoPtr temp = actual;
...
void dibuja_lista_balas ( NodoPtr cabecera ) {
    ...
    while (aux != NULL) {
        dibuja_imagen(aux->elem->img, aux->elem->x, aux->elem->y, aux->elem->w, aux->elem->h);
        aux = aux->sig;
    }
}
